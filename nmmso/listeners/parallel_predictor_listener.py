...
class ParallelPredictorListener(BaseListener):
    ...
    def iteration_ended(self, n_new_locations, n_mid_evals, n_evol_modes, n_rand_modes, n_hive_samples):
        self.total_evaluations += n_new_locations + n_mid_evals + n_evol_modes + n_rand_modes + n_hive_samples
    ...
    def print_summary(self):
        ...
        for i in range(len(self.num_workers)):
            print("{} workers, effective evaluations {}, minimum runtime as percentage of sequential runtime: {:0.2f}%".format(
                self.num_workers[i], self.min_evaluations[i], self.min_evaluations[i]/self.total_evaluations * 100.0))
